...
function oncooldown()
	...
	for v, k in pairs(details) do
		...
		if not k.passive and k.cooldown and not RecastBoxSettings.Filtered[v] then
			...
			if k.currentCooldown and k.currentCooldownRemaining then
				if math.floor(k.cooldown) == math.floor(k.currentCooldown) then
					if index[v] then
						RecastBox.UI.Update(v, k.currentCooldownRemaining)
					else
...
table.insert(Event.Ability.AvailabilityChange, {onavailability, RecastBox.Version.Name, "OnAvailability"})